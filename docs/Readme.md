...
Make sure the python version is Python 3.10.12, you can check this by running python -V.
If you have Windows OS then it is better to download Python 3.10.11
...
We use a document on Google sheets and we push into it using the API provided by google. Follow these steps to make it work
...
You will find a file called `.env.example`, rename the file to `.env` modify the following values with the ones corresponding to thouse of your machine

```
UNITY_ENV_PATH=your-unity-build-path
...
Create a new build with the desired scene and change accordingly the values in the `.env` file. A new run id is created automatically for each run.

Then run:
```
python run_training.py < desired number of trainings >
```

Example with 10 trainings:
```
